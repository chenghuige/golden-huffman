...
public:
  Buffer(FILE* file, int buf_capacity = 64 * 1024) 
      :file_(file), cur_(0), bit_cur_(0), num_(0), 
       buf_capacity_(buf_capacity), buf_size_(0)
  {
  ...
  int fill_buf() {
    cur_ = 0;
    buf_size_ = fread(buf_, 1, buf_capacity_, file_);
    return buf_size_;
  }
  ...
  //for writing string or bits,the last byte,return still nedd how many bits to fill
  int left_bits() {
    return (8 - bit_cur_) % 8;
  }
